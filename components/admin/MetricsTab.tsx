...
export const MetricsTab: React.FC<MetricsTabProps> = ({ 
  ...
  const chartData = useMemo(() => {
    ...
    const dataPoints = Object.entries(countsByDay).map(([date, count]) => ({ date, count })).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  const renderPostsView = () => (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div>
            <h3 className="text-xl font-oswald font-black italic uppercase text-neon mb-4">Publicaciones por Cuentas</h3>
         ...
         <div>
            <h3 className="text-xl font-oswald font-black italic uppercase text-neon mb-4">Publicaciones por Autor</h3>
    ...
    <div><h3 className="text-xl font-oswald font-black italic uppercase text-neon mb-4">Tabla de Publicaciones</h3><div className="overflow-x-auto bg-white/5 rounded-lg border border-white/10"><table className="w-full text-sm text-left"><thead className="bg-white/5 text-xs text-gray-400 uppercase"><tr className="border-b border-white/10"><th className="px-4 py-3">Publicación</th><th className="px-4 py-3">Autor</th><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Cuentas</th><th className="px-4 py-3">Sponsors</th><th className="px-4 py-3 text-center">Estado</th></tr></thead><tbody>{filteredPosts.map(post => (<tr key={post.id} onClick={() => onOpenDetail(post)} className="border-b border-white/5 hover:bg-white/10 transition-colors cursor-pointer"><td className="px-4 py-2"><div className="flex items-center gap-3"><img src={post.imageUrl} alt={post.titleOverlay} className="w-10 h-10 object-cover rounded flex-shrink-0" /><span className="font-bold text-white truncate max-w-xs">{post.titleOverlay}</span></div></td><td className="px-4 py-2 text-gray-300">{users.find(u => u.id === post.postedBy)?.name || 'N/A'}</td><td className="px-4 py-2 text-gray-400">{post.postedAt ? new Date(post.postedAt).toLocaleDateString('es-AR') : 'S/F'}</td><td className="px-4 py-2"><div className="flex items-center -space-x-2">{(post.postedToAccounts || []).map(id => socialAccountMap.get(id)).filter((acc): acc is SocialAccount => !!acc).map(acc => (<img key={acc.id} src={acc.profileImageUrl} title={acc.name} className="w-6 h-6 rounded-full border-2 border-black" />))}</div></td><td className="px-4 py-2"><div className="flex items-center -space-x-2">{(post.associatedSponsors || []).map(id => brandMap.get(id)).filter((brand): brand is Brand => !!brand).map(brand => (<img key={brand.id} src={brand.logoUrl} title={brand.name} className="w-6 h-6 rounded-full border-2 border-black bg-neutral-800 p-0.5 object-contain" />))}</div></td><td className="px-4 py-2 text-center"><span className={`px-2 py-0.5 text-[9px] font-black uppercase rounded-full ${post.status === 'success' ? 'bg-green-500/20 text-green-300' : post.status === 'draft' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-red-500/20 text-red-300'}`}>{post.status}</span></td></tr>))}</tbody></table></div></div></>
  );
    ...
    <div>
      <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
        ...
        <div className="flex items-center bg-black/50 border border-white/10 rounded-lg p-1">
          ...
          <button onClick={() => setView('posts')} className={`px-4 py-1.5 text-xs font-black uppercase rounded-md transition ${view === 'posts' ? 'bg-neon text-black' : 'text-gray-400 hover:bg-white/10'}`}>Publicaciones</button>
        </div>
      </div>
