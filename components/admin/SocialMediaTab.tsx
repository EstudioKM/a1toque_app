...
export const SocialMediaTab: React.FC<SocialMediaTabProps> = ({ 
  ...
  const [activeSubTab, setActiveSubTab] = useState<'workbench' | 'history'>('workbench');
  ...
  const selectedAccount = useMemo(() => 
    socialAccounts.find(acc => acc.id === selectedAccountId), 
    ...
    <div className="max-w-6xl mx-auto pb-24 px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        ...
        <div className="flex bg-black/40 p-1 rounded-2xl border border-white/5 self-stretch md:self-auto">
          <button 
            ...
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase italic tracking-widest transition-all ${activeSubTab === 'workbench' ? 'bg-neon text-black shadow-[0_0_20px_rgba(0,255,157,0.2)]' : 'text-gray-500 hover:text-white'}`}
          >
            <Zap size={14} /> WORKBENCH
          </button>
          <button 
      ...
      {activeSubTab === 'workbench' ? (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="bg-[#0f0f0f] p-6 rounded-[32px] border border-white/5 flex flex-col justify-between">
                <div>
                    ...
                    <div className="flex -space-x-2 overflow-hidden mb-4">
                        {socialAccounts.slice(0, 5).map(acc => (
                            <img key={acc.id} src={acc.profileImageUrl} className="w-8 h-8 rounded-full border-2 border-[#0f0f0f] bg-neutral-800" title={acc.name} />
                        ))}
                ...
                <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">Gestionando {socialAccounts.length} perfiles sociales</p>
             </div>
          ...
          {/* Input de Generación */}
          <div className="bg-[#0f0f0f] p-8 rounded-[40px] border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center justify-between mb-6 px-2">
                ...
                <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full">{socialGenerationQueue.length} TAREAS</span>
            </div>

            <div className="grid grid-cols-1 gap-4">
              {socialGenerationQueue.length === 0 ? (
